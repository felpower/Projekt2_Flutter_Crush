...
def get_random_message():
    messages = [
        "Hallo! Hast du heute schon gespielt?",
        "Hey! Wie wäre es mit einer Runde JellyFun?",
        "JellyFun wartet auf dich!",
        "JellyFun vermisst dich!,"
        "Hey! Hast du heute schon JellyFun gespielt?",
        "JellyFun ist bereit für dich!"]
    ...
    for token_info in tokens:
        try:
            if token_info['dark_patterns'] == 1 and not token_info.get('survey_filled', False):
                send_message(token_info['token'], get_random_message())
