...
class GameController {
  ...
  //
  final HashMap<int, Swap> _swaps = HashMap<int, Swap>();
  ...
  void shuffle(String powerUp) {
    ...
    do {
      ...
      int powerUpRow = -1;
      int powerUpCol = -1;
      TileType? powerUpTile = Tile.random(_rnd);
      if (powerUp != "nothing") {
      ...
      //
      identifySwaps();
      if (-1 != powerUpRow && -1 != powerUpCol) {
        _grid[powerUpRow][powerUpCol] = Tile(
            row: powerUpRow,
            col: powerUpCol,
            type: powerUpTile,
            level: level,
