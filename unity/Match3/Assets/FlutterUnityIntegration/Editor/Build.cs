...
namespace FlutterUnityIntegration.Editor {
	public class Build : EditorWindow {
		...
		private void OnEnable() { _pluginMode = EditorPrefs.GetBool(_persistentKey, false); }
		...
		private void OnGUI() {
			...
			if (EditorGUI.EndChangeCheck()) EditorPrefs.SetBool(_persistentKey, _pluginMode);
		}
		...
		public static void DoBuildIOSDebug() { BuildIOS(IOSExportPath, false); }

		[MenuItem("Flutter/Export IOS (Release) %&i", false, 202)]
		public static void DoBuildIOSRelease() { BuildIOS(IOSExportPath, true); }
		...
		public static void DoBuildWebGL() { BuildWebGL(WebExportPath); }


		[MenuItem("Flutter/Export Windows %&d", false, 401)]
		public static void DoBuildWindowsOS() { BuildWindowsOS(WindowsExportPath); }

		[MenuItem("Flutter/Settings %&S", false, 501)]
		public static void PluginSettings() { GetWindow(typeof(Build)); }
		//#endregion
...
#unity-mobile-warning { position: absolute; left: 50%; top: 5%; transform: translate(-50%); background: white; padding: 10px; display: none }
		...
		private static void ModifyAndroidGradle(bool isPlugin) {
			...
			if (isPlugin) buildText = Regex.Replace(buildText, @"implementation\(name: 'androidx.* ext:'aar'\)", "\n");
		...
		private static void BuildIOS(string path, bool isReleaseBuild) {
			...
			if (!isReleaseBuild) playerOptions.options = BuildOptions.AllowDebugging | BuildOptions.Development;
