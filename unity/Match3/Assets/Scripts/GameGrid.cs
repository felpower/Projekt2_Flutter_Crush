            ...
            for (int x = 0; x < xDim; x++) {
                for (int y = 0; y < yDim; y++) {
                    if (_pieces[x, y].IsClearable())
                        _pieces[x, y].ClearableComponent.Clear(false);
                }
            }
        }
        ...
        private bool GetMatchHorizontal(GamePiece piece1, GamePiece piece2)
        {
            ...
            for (int dir = 0; dir <= 1; dir++) {
                for (int xOffset = 1; xOffset < xDim; xOffset++) {
                    ...
                    if (_pieces[x, piece2.Y].IsColored() && _pieces[x, piece2.Y].ColorComponent.Color == color) {
                        horizontalPieces.Add(_pieces[x, piece2.Y]);
                    } else {
                        break;
                    }
                }
        ...
        private bool GetMatchVertical(GamePiece piece1, GamePiece piece2)
        {
            ...
            for (int dir = 0; dir <= 1; dir++) {
                for (int yOffset = 1; yOffset < xDim; yOffset++) {
                    ...
                    // piece is the same color?
                    if (_pieces[piece2.X, y].IsColored() && _pieces[piece2.X, y].ColorComponent.Color == color) {
                        verticalPieces.Add(_pieces[piece2.X, y]);
                    } else {
                        break;
                    }
                }
